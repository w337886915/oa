...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
    <td class="Big"><img src="template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 客户回访信息列表</span>
	<span style="font-size:12px; float:right; margin-right:20px;"><input type="button" value="创建任务" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=<?php echo $ac;?>&do=add&fileurl=<?php echo $fileurl;?>&cid=<?php echo $cid;?>'">
	</span>
    </td>
  </tr>
</table>
...
<label class="ui-form-label" for="keyword">回访主题：</label>
							<div id="J-keyword-type-select" class="ui-form-select-shell">
...
<?php foreach ($result as $row) {?>
	...
	?></td>
	  ...
	  <?php form_list_view('crm_service',$row['id']);?>
	  <td align="center" class="TableData">
      <?php echo $row['startdate']?></td>
	  <td align="center" class="TableData">
      <?php echo str_replace(' ','<br>',$row['enddate'])?></td>
	  <td align="center" class="TableData">
      ...
      <td align="center" class="TableData"><a href="admin.php?ac=<?php echo $ac;?>&fileurl=<?php echo $fileurl;?>&do=view&id=<?php echo $row['id']?>&cid=<?php echo $row['cid']?>">查看</a>
<?php if((trim(get_realname($_USER->id))==trim($row['user']) || $_USER->id==$row['uid']) && $row['enddate']==''){?> | <a href="admin.php?ac=<?php echo $ac;?>&fileurl=<?php echo $fileurl;?>&do=edit&id=<?php echo $row['id']?>" style="color:#FF0000;">回访</a>
<?php }?></td>
    </tr>
...
<?php }?>	
    ...
    <tr align="center" class="TableControl">
      <td height="35" colspan="<?php echo 10+$fromnum;?>" align="left" nowrap>
