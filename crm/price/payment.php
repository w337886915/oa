...
<table width="98%" border="0" align="center" cellpadding="3" cellspacing="0" class="small">
  <tr>
    <td class="Big"><img src="template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 付款信息列表</span>
	<span style="font-size:12px; float:right; margin-right:20px;"><input type="button" value="新增付款" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=<?php echo $ac;?>&do=add&fileurl=<?php echo $fileurl;?>'">
	</span>
    </td>
  </tr>
</table>
...
<label class="ui-form-label" for="keyword">付款名称：</label>
							<div id="J-keyword-type-select" class="ui-form-select-shell">
...
<?php foreach ($result as $row) {?>
	...
	?></td>
	  ...
	  <?php form_list_view('crm_payment',$row['id']);?>
	  ...
	  <?php
		if($_flow==1){
			crm_flow1($row['id'],'crm_payment');
		}
		?>
      <td align="center" class="TableData">
	  <?php 
		if($_flow==1){
			crm_flow2($row['id'],'crm_payment',$fileurl);
		}
		?>
