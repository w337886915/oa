...
<a href="admin.php?ac=<?php echo $ac;?>&fileurl=<?php echo $fileurl;?>&do=edit&id=<?php echo $view['id'];?>"><span><img src="template/default/content/images/p2.gif" width="16" height="16" align="absmiddle">产品编辑</span></a>
<a href="admin.php?ac=<?php echo $ac;?>&fileurl=<?php echo $fileurl;?>&do=view&id=<?php echo $view['id'];?>" class="active"><span><img src="template/default/content/images/p4.gif" width="16" height="16" align="absmiddle">产品查看</span></a>

<a href="#" onClick="window.open ('admin.php?ac=public&do=crmlog&fileurl=crm&viewid=<?php echo $view['id'];?>&modid=crm_product', '<?php echo $view['id'];?>', 'height=600, width=600, top=50, left=150, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no')"><span><img src="template/default/images/admin_img/detail.png" width="16" height="16" align="absmiddle">操作记录</span></a>
</div>
<div id="search" style="float: right; padding-right:100px;">
	
	<input type="button" value="发布产品" class="BigButtonBHover" onClick="javascript:window.location='admin.php?ac=<?php echo $ac;?>&do=add&fileurl=<?php echo $fileurl;?>'">
...
<tr>
	...
	<tr>
      ...
      <td class="TableContent" width="15%"> 产品名称：</td>
      <td class="TableData" width="35%"><?php echo $view['title'];?>
       </td>
    </tr>
	<tr>
      <td nowrap class="TableContent">产品价格：</td>
      <td class="TableData"><span style="font-size:18px; color:#FF0000; font-weight:900;"><?php echo $view['price'];?></span>元</td>
      <td class="TableContent" width="15%">产品分类：</td>
      <td class="TableData" width="35%">
	  <?php
	  if($view['type']!=''){
		  echo public_value('title','crm_pord_type','id='.$view['type']);
	  }
	  ?> 
      </td>
    </tr>
...
<?php
//引入表单
form_view('产品信息','crm_product',$view['id']);
//引入编辑器
form_view_eweb('crm_product',$view['id']);
?>
