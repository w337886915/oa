...
if($step=='2'){
	...
	if($dbbackup->import("mysql.sql")){				//导入数据
		show_msg('内部邮件模块安装成功！', '../admin.php?ac=plugin&fileurl=mana');
	}
}elseif($step=='1'){
	global $db;
	$db->query("UPDATE ".DB_TABLEPRE."plugin set type='1' WHERE id = '".$_POST['pid']."' ");
	//菜单
	$db->query("DELETE FROM ".DB_TABLEPRE."menu WHERE menuid = 303");
	//数据库
	$db->query("DROP TABLE toa_recemail");
	$db->query("DROP TABLE toa_sendmail");
	$db->query("DROP TABLE toa_popmail");
	$db->query("DROP TABLE toa_mailsignature");
	$db->query("DROP TABLE toa_emailtype");
	show_msg('内部邮件模块卸载成功！', '../admin.php?ac=plugin&fileurl=mana');
}else{
	global $db;
...
<table width="60%" border="0" align="center" cellpadding="3" cellspacing="0" class="small" style="margin-top:20px;margin-bottom:10px;">
  <tr>
    <td class="Big"><img src="../template/default/content/images/notify_new.gif" align="absmiddle"><span class="big3"> 内部邮件集成组件安装</span>&nbsp;&nbsp;&nbsp;&nbsp;
