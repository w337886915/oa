...
<form name="update" method="post" action="admin.php?ac=<?php echo $ac?>&fileurl=<?php echo $fileurl?>&typeid=<?php echo $_GET['typeid']?>">
...
?>
      <td>
        <a href="admin.php?ac=<?php echo $ac;?>&do=view&fileurl=<?php echo $fileurl;?>&id=<?php echo $row['id'];?>"><?php echo $row['subject']?></a>
      </td>
	  <td nowrap align="center" ><?php echo get_realname($row['user'])?></td>
      <td nowrap ><?php echo str_replace(' ','<br>',$row['date']);?></td>
      <td align="center" ><?php if($row['type']=='0'){echo "未读";}elseif($row['type']=='1'){echo "<font color=red>己读</font>";}else{echo "<font color=red>己删除</font>";}?></td>
    </tr>
<?php } ?>	       
    </table>
</div>
